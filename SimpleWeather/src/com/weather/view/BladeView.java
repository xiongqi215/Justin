...
public class BladeView extends View {
	...
	private void showPopup(int item) {
		if (popupWindow == null) {
			...
			popupText.setBackgroundColor(Color.GRAY);
			popupText.setTextColor(Color.CYAN);
			popupText.setTextSize(50);
			popupText.setGravity(Gravity.CENTER_HORIZONTAL
					| Gravity.CENTER_VERTICAL);
			popupWindow = new PopupWindow(popupText, 100, 100);
		}
	...
	private void performItemClicked(int item) {
		if (onItemClickListener != null) {
			onItemClickListener.onItemClick(b[item]);
			showPopup(item);
		}
	}
